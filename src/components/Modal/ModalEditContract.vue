<template>
  <form action="" @submit.prevent="handleSubmit">
    <div class="grid grid-cols-12 gap-6">
      ...
      <!-- Họ tên và ngày sinh không cần thiết khi sửa HĐ, giữ giống modal thêm -->
...
<script setup lang="ts">
  ...
  watch(
    ...
    (newVal) => {
      ...
      // refresh user list when opening/receiving new data
      fetchUserID()

      params.id = (data.id ?? '').toString()
      params.user_id = (data.user_id ?? '').toString()
      params.name = (data.name ?? '').toString()
      params.salary = (data.salary ?? '').toString()
      params.probationary_salary = (data.probationary_salary ?? '').toString()
      params.probationary_rate = (data.probationary_rate ?? '').toString()
      params.contract_name = (data.contract_name ?? '').toString()
      params.contract_type = (data.contract_type ?? '').toString()
      params.contract_term = (data.contract_term ?? '').toString()
      params.birthday = (data.birthday ?? '').toString()
      params.join_date = (data.join_date ?? '').toString()
      params.begin_date = (data.begin_date ?? '').toString()
      params.finish_date = (data.finish_date ?? '').toString()

      birthdayPicker.value = data.birthday ? new Date(data.birthday) : null
      joinDatepicker.value = data.join_date ? new Date(data.join_date) : null
      beginDatepicker.value = data.begin_date ? new Date(data.begin_date) : null
      finishDatepicker.value = data.finish_date ? new Date(data.finish_date) : null
    },
    { immediate: true }
  )
