<template>
  <SelectRoot :model-value="modelValue" @update:model-value="updateModelValue">
    ...
    <SelectPortal>
      ...
      >
        <div v-if="showSearch" class="sticky top-0 z-10 border-b border-gray-200 bg-[#FAFAFA] p-2">
          <input
            ref="searchInput"
            v-model="searchQuery"
            type="text"
        ...
        <SelectViewport :style="{ maxHeight }">
          <SelectGroup>
            ...
            <!-- No results message -->
            <div
              v-if="showSearch && searchQuery.trim() && filteredOptions.length === 0"
...
<script setup lang="ts">
  ...
  const filteredOptions = computed(() => {
    if (!props.showSearch || !debouncedSearchQuery.value.trim()) {
