<template>
  <MainLayout>
    ...
    <div class="mb-3 flex flex-wrap items-center gap-2">
      ...
      <template v-if="checkPermission('Contract', 'Create')">
    ...
    <Modal :modal-active="modalActive.modalAddLeave" max-width="max-w-[670px]" @close="toggleModal('modalAddLeave')">
      <div class="rounded-[24px] bg-white p-[52px_24px_36px]">
        <div class="mb-12 text-center max-xl:mb-6">
          <h3 class="m-0 text-[16px] font-bold text-[#464661] uppercase">thêm mới hợp đồng</h3>
        </div>

        <ModalAddContract
          :datatype="null"
          :prop-function="fetchDataContract"
          :is-open="modalActive.modalAddLeave"
          :is-refetch-list="fetchDataContract"
    ...
    <Modal
      :modal-active="modalActive.modalRemoveContract"
      max-width="max-w-[670px]"
      @close="toggleModal('modalRemoveContract')"
    >
      <div class="flex min-h-[470px] flex-col items-center justify-center rounded-[24px] bg-white p-[52px_24px_36px]">
        <div class="text-center">
          <h3 class="m-0 text-[16px] font-bold text-[#464661] uppercase">THANH LÝ HỢP ĐỒNG</h3>
        </div>

        <ModalRemoveContract
          :datatype="{ id: contractToRemove }"
          :prop-function="fetchDataContract"
          @post-request-remove="getPostRequestRemove"
        >
          <button
            type="button"
            class="hover:shadow-hoverinset inset-sha inline-block cursor-pointer rounded-[8px] border border-solid border-[#EDEDF6] bg-white p-2 text-center text-[16px] leading-normal font-bold text-[#464661] uppercase transition hover:transition max-md:grow md:min-w-[175px]"
            @click="toggleModal('modalRemoveContract')"
          >
            Hủy
          </button>
        </ModalRemoveContract>
    ...
    <Modal
      :modal-active="modalActive.modalEditContract"
      max-width="max-w-[670px]"
      ...
      <div class="overflow-hidden rounded-[24px] bg-white p-[52px_24px_36px]">
        ...
        >
          <button
            type="button"
            class="hover:shadow-hoverinset inset-sha inline-block cursor-pointer rounded-[8px] border border-solid border-[#EDEDF6] bg-white p-2 text-center text-[16px] leading-normal font-bold text-[#464661] uppercase transition hover:transition max-md:grow md:min-w-[175px]"
            @click="toggleModal('modalEditContract')"
          >
            Hủy
          </button>
        </ModalEditContract>
    ...
    >
      <ModalDeleteContract @cancel="toggleModal('modalStatusConfirm')" @confirm="submitDeleteContract" />
    </Modal>
    ...
    <ToastProvider>
      <ToastRoot
        v-model:open="toast.toastRemove"
        :duration="5000"
        class="flex flex-col gap-1.5 rounded-md p-3 shadow-2xl"
      >
        <ToastTitle class="text-[13px] font-medium">
          {{ dataPostRequestRemove?.message }}
        </ToastTitle>
        <ToastDescription v-if="dataPostRequestRemove?.errors" class="text-[11px] font-normal">
          {{ dataPostRequestRemove?.errors[Object.keys(dataPostRequestRemove?.errors)[0]] }}
        </ToastDescription>
      </ToastRoot>
      <ToastViewport
        class="fixed right-0 bottom-0 z-[2147483647] m-0 flex w-[390px] max-w-[100vw] list-none flex-col gap-[10px] p-[var(--viewport-padding)] outline-none [--viewport-padding:_25px]"
...
<script lang="ts" setup>
  ...
  const tbhead = reactive([
    { title: 'STT', hasSort: false },
    { title: 'Mã NV', hasSort: false },
    { title: 'Họ và tên', hasSort: false },
    { title: 'Tên HĐ', hasSort: false },
    { title: 'Loại HĐ', hasSort: false },
    { title: 'Thời hạn HĐ', hasSort: false },
    { title: 'Ngày bắt đầu', hasSort: false },
    { title: 'Ngày kết thúc', hasSort: false },
    { title: 'Ngày thanh lý', hasSort: false },
  ])
  ...
  const contractToRemove = ref<number | null>(null)
  const confirmDeleteLeave = (id: number) => {
    contractToRemove.value = id
    toggleModal('modalRemoveContract')
  }

  const idToDeleteContract = ref<number | null>(null)
  const handleDeleteContract = async (id: number) => {
    idToDeleteContract.value = id
  ...
  // Legacy binding kept previously for leave pages is removed in contract page
