...
<template>
  <MainLayout>
    ...
    <div class="mb-3">
      <div class="grid grid-cols-12 gap-6">
        <div class="col-span-12 lg:col-span-6">
          <div class="search-box-cpn rounded-[24px] bg-white p-2.5">
            <form class="flex flex-wrap items-stretch gap-2" @submit.prevent="handleSearchReport">
              <div class="grow">
                <VueDatePicker
                  v-model="datepicker"
    ...
    <div class="grid grid-cols-12 gap-6">
      <div v-for="(item, id) in ticket.slice(-2)" :key="id" class="col-span-12 lg:col-span-6">
        <Ticket
          :ticket-props="[item.icon, item.title, item.ticketList]"
          :class="[route.path == '/dashboard/report' ? 'ticketbox-dashboard-report' : '']"
        />
      </div>
    </div>
  </MainLayout>
