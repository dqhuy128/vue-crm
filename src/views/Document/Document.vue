  ...
  <MainLayout>
    ...
    <template v-if="toggleBoxFilters">
      <div class="mb-5 rounded-[24px] bg-white p-2.5">
        <form class="xxl:gap-4 flex flex-wrap items-stretch gap-2" @submit.prevent="handleSearchDocument">
                  ...
                  <template v-for="(item, _) in document">
                    ...
                    <template v-if="permissionList">
                      <div class="cell pinned pinned-body">
                        <div class="cell edit edit-body justify-end">
                          <template v-if="item.link">
                            <a :href="item.link" target="_blank" class="cell-btn-view me-auto shrink-0 cursor-pointer">
                              <img src="@/assets/images/action-edit-1.svg" alt="" />
                            </a>
                          </template>
                          <template v-if="checkPermission('Document', 'Update') && checkEditId(item.user_id)">
                            <button
                              type="button"
                          ...
                          <template v-if="checkPermission('Document', 'Delete') && checkEditId(item.user_id)">
                            <button
                              type="button"
    ...
    <Modal
      :modal-active="modalActive.modalAddDocument"
    ...
    >
      <div class="overflow-hidden rounded-[24px] bg-white p-[52px_24px_36px]">
        <div class="mb-12 text-center max-xl:mb-6">
          <h3 class="m-0 text-[16px] font-bold text-[#464661] uppercase">thêm mới tài liệu</h3>
        </div>

        <CreateDocument
          :prop-function="fetchDataDocument"
          :prop-data="dataTypeDocument.data"
    ...
    <Modal
      :modal-active="modalActive.modalEditDocument"
      max-width="max-w-[670px]"
      @close="toggleModal('modalEditDocument')"
    >
      <div class="overflow-hidden rounded-[24px] bg-white p-[52px_24px_36px]">
        <div class="mb-12 text-center max-xl:mb-6">
          <h3 class="m-0 text-[16px] font-bold text-[#464661] uppercase">Update tài liệu</h3>
        </div>

        <EditDocument
          :close-modal="() => toggleModal('modalEditDocument')"
          :prop-function="fetchDataDocument"
          :data="detailDocument"
          :prop-data="dataTypeDocument.data"
...
<script lang="ts" setup>
  ...
  // Get loading state from useDocument
