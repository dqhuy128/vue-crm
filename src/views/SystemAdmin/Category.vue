  ...
  <MainLayout>
    ...
    <template v-if="toggleBoxFilters">
      <div class="mb-5 rounded-[24px] bg-white p-2.5">
        <form class="xxl:gap-4 flex flex-wrap gap-2" @submit.prevent="handleSearchDocument">
                    ...
                    <SelectViewport>
                      <SelectGroup>
                        <SelectItem
                          ...
                          value="0"
                        >
                          <SelectItemText> Tất cả loại danh mục </SelectItemText>
                        </SelectItem>
                        <SelectItem
                ...
                <div v-for="(item, index) in dataDocument.doc.items" :key="index" class="table-item justify-between">
                  <template v-for="(it, _) in item">
                    ...
                    <template v-if="Number(it.status) === 0 || it.status === null">
                      <div class="cell status status-red status-body">Dừng hoạt động</div>
                    </template>
    ...
    >
      <div class="overflow-hidden rounded-[24px] bg-white p-[52px_24px_36px]">
        <div class="mb-12 text-center max-xl:mb-6">
          <h3 class="m-0 text-[16px] font-bold text-[#464661] uppercase">thêm mới danh mục</h3>
        </div>

        <ModalAddCategory :datatype="selectData" :prop-function="fetchDataDocument" @post-request="getPostRequest">
          <button
            type="button"
    ...
    >
      <div class="overflow-hidden rounded-[24px] bg-white p-[52px_24px_36px]">
        <div class="mb-12 text-center max-xl:mb-6">
          <h3 class="m-0 text-[16px] font-bold text-[#464661] uppercase">Cập nhật danh mục</h3>
        </div>
